...
public class ProfilePicGuesserRegularMedium : MonoBehaviour
{
    ...
    public Image[] gameImages = new Image[10];
    ...
    public void Start()
    {
        ...
        for (int i = 0; i < 80; i++)
        {
            ...
            // Tiles[throwaway2[i]].anchoredPosition = new Vector3(xCoords[i], yCoords[i]);
        ...
        gameImages[0].sprite = pictures[throwaway2[currentPlaceInOrder]];
        whatIs.text = "What is " + pictures[throwaway2[currentPlaceInOrder]].name + "'s profile picture?";

        // at this point, the correct image is in the first tile

        usernamesToDisplay1[throwaway2[currentPlaceInOrder]] = true;    // to make sure that there is no duplication of the correct image one of the incorrect tiles
        ...
        for (int i = 1; i < 10; i++)
        {
            ...
            gameImages[i].sprite = pictures[throwaway3[i]];
            Debug.Log("The picture (" + throwaway3[i] + ") is in the tile (" + i + ").");     // displays in the console which picture is in which tile
        }

        // at this point, all tiles from 2 to 10 now have random wrong pictures
    ...
    public void Update()
    {
        ...
        {
            highScoreMatches.text = "Matches: " + Settings.highestProPicGuesserSuddenDeathMediumMatchesScore.ToString();
        }
        else
        {
            mistakesText.text = "Mistakes: " + mistakes.ToString();
            highScoreMatches.text = "Matches: " + Settings.highestProPicGuesserRegularMediumMatchesScore.ToString();
            highScoreMistakes.text = "Mistakes: " + Settings.highestProPicGuesserRegularMediumMistakesScore.ToString();
        }
    ...
    {
        mistakes++;
        Settings.totalMistakesInProPicGuesser++;
    ...
    {
        matches++;
        Settings.totalMatchesInProPicGuesser++;
    ...
    public void GameOver()
    {
        ...
        if (matches > Settings.highestProPicGuesserRegularMediumMatchesScore)
        {
            newHighScore.SetActive(true);
            Settings.highestProPicGuesserRegularMediumMatchesScore = matches;
            Settings.highestProPicGuesserRegularMediumMistakesScore = mistakes;
        }
        else if (matches == Settings.highestProPicGuesserRegularMediumMatchesScore && mistakes < Settings.highestProPicGuesserRegularMediumMistakesScore)
        {
            newHighScore.SetActive(true);
            Settings.highestProPicGuesserRegularMediumMistakesScore = mistakes;
        }

        Settings.totalProPicGuesserRegularMediumGames++;
    ...
    public void ResetTheBoard()
    {
        ...
        // Allows every image to be able to be chosen to go on the board
        for (int i = 0; i < 80; i++)
        {
    ...
    public void SDGameOver()
    {
        ...
        if (matches > Settings.highestProPicGuesserSuddenDeathMediumMatchesScore)
        {
            newHighScore.SetActive(true);
            Settings.highestProPicGuesserSuddenDeathMediumMatchesScore = matches;
        }

        Settings.totalProPicGuesserSuddenDeathMediumGames++;
        Invoke("ShowGameOverPanel", 0.6f);
    }
}